...
<body>
<div id="container">
  <div id="menu"><a id="create" class="button" style="background-image: url('catalog/view/theme/pav_queenbeauty/image/filemanager/folder.png');"><?php echo $button_folder; ?></a><a id="delete" class="button" style="background-image: url('catalog/view/theme/pav_queenbeauty/image/filemanager/edit-delete.png');"><?php echo $button_delete; ?></a><a id="move" class="button" style="background-image: url('catalog/view/theme/pav_queenbeauty/image/filemanager/edit-cut.png');"><?php echo $button_move; ?></a><a id="copy" class="button" style="background-image: url('catalog/view/theme/pav_queenbeauty/image/filemanager/edit-copy.png');"><?php echo $button_copy; ?></a><a id="rename" class="button" style="background-image: url('catalog/view/theme/pav_queenbeauty/image/filemanager/edit-rename.png');"><?php echo $button_rename; ?></a><a id="upload" class="button" style="background-image: url('catalog/view/theme/pav_queenbeauty/image/filemanager/upload.png');"><?php echo $button_upload; ?></a><a id="refresh" class="button" style="background-image: url('catalog/view/theme/pav_queenbeauty/image/filemanager/refresh.png');"><?php echo $button_refresh; ?></a></div>
...
$(document).ready(function() { 
	...
	$('#column-right').bind('scrollstop', function() {
		$('#column-right a').each(function(index, element) {
			...
			if ((offset.top > 0) && (offset.top < height) && $(element).find('img').attr('src') == '<?php echo $no_image; ?>') {
				$.ajax({
					url: 'index.php?route=myproducts/filemanager/image&token=<?php echo $token; ?>&image=' + encodeURIComponent('data/' + $(element).find('input[name=\'image\']').attr('value')),
					dataType: 'html',
	...
	$('#column-left').tree({
		data: { 
			...
			opts: { 
				method: 'post', 
				url: 'index.php?route=myproducts/filemanager/directory&token=<?php echo $token; ?>'
			} 
		},
		selected: 'top',
		ui: {		
		...
		callback: {
			...
			onselect: function (NODE, TREE_OBJ) {
				$.ajax({
					url: 'index.php?route=myproducts/filemanager/files&token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#create').bind('click', function() {
		...
		if (tree.selected) {
			...
			$('#dialog input[type=\'button\']').bind('click', function() {
				$.ajax({
					url: 'index.php?route=myproducts/filemanager/create&token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#delete').bind('click', function() {
		...
		if (path) {
			$.ajax({
				url: 'index.php?route=myproducts/filemanager/delete&token=<?php echo $token; ?>',
				type: 'post',
	...
	$('#move').bind('click', function() {
		...
		$('#dialog select[name=\'to\']').load('index.php?route=myproducts/filemanager/folders&token=<?php echo $token; ?>');
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {																
				$.ajax({
					url: 'index.php?route=myproducts/filemanager/move&token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#copy').bind('click', function() {
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {																
				$.ajax({
					url: 'index.php?route=myproducts/filemanager/copy&token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#rename').bind('click', function() {
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {		
				$.ajax({
					url: 'index.php?route=myproducts/filemanager/rename&token=<?php echo $token; ?>',
					type: 'post',
					data: 'path=' + encodeURIComponent(path) + '&name=' + encodeURIComponent($('#dialog input[name=\'name\']').val()),
					dataType: 'json',
					success: function(json) {
	...
	new AjaxUpload('#upload', {
		action: 'index.php?route=myproducts/filemanager/upload&token=<?php echo $token; ?>',
		name: 'image',
		autoSubmit: false,
		responseType: 'json',
